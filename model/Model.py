...
from mindspore.common.initializer import XavierNormal, Zero, One, initializer, Uniform
...
class MyTrainOneStepCell(nn.Cell):
    ...
    def construct(self, *inputs):
        loss, grads = ms.value_and_grad(self.network, grad_position=None, weights=self.network.trainable_params())(*inputs)
        # loss = self.network(*inputs)
        # grads = self.grad(self.network)(*inputs)
        # grads = ms.ops.clip_by_global_norm(grads, clip_norm=10.0)
        grads = ms.ops.clip_by_value(grads, clip_value_max=1.0)
...
class Model(nn.Cell):
    ...
    def __init__(self, numOfEntity, numOfRelation, numOfhidden, dataset, raw_ent, raw_rel, init_type):
        ...
        self.relation_embeddings_1 = nn.Embedding(self.numOfRelation + 1, self.numOfhidden,
                                                  padding_idx=self.numOfRelation)
        self.relation_embeddings_1.embedding_table.data[0:numOfRelation] = ms.ops.uniform(
            shape=(self.numOfRelation, self.numOfhidden), dtype=ms.float32, minval=ms.Tensor(-1. / sqrtR, ms.float32),
            maxval=ms.Tensor(1. / sqrtR, ms.float32))
        ...
        self.entity_weights = nn.Embedding(self.numOfEntity + 1, self.numOfbase, padding_idx=self.numOfEntity)
        self.entity_weights.embedding_table.data[0:self.numOfEntity] = ms.ops.uniform(
            shape=(self.numOfEntity, self.numOfbase), dtype=ms.float32, minval=ms.Tensor(-1. / sqrtE, ms.float32),
            maxval=ms.Tensor(1. / sqrtE, ms.float32))
        ...
        self.entity_weights.embedding_table.requires_grad = False
        ...
        self.base_matrix = nn.Dense(self.numOfbase, self.numOfhidden, has_bias=False, weight_init='xavier_normal')
        self.update_matrix = nn.Dense(2 * self.numOfhidden, self.numOfhidden, has_bias=False,
                                      weight_init='xavier_normal')
        self.build_matrix = nn.Dense(2 * self.numOfhidden, self.numOfhidden, has_bias=False,
                                     weight_init='xavier_normal')
        self.dir_influence_matrix = nn.Dense(2 * self.numOfbase, self.numOfbase, has_bias=False,
                                             weight_init='xavier_normal')
        self.prop_influence_matrix = nn.Dense(3 * self.numOfbase, self.numOfbase, has_bias=False,
                                              weight_init='xavier_normal')
        self.gate_matrix = nn.Dense(3 * self.numOfbase, self.numOfbase, has_bias=False, weight_init='xavier_normal')
        self.project_matrix = nn.Dense(2 * self.numOfbase, self.numOfbase, has_bias=True, weight_init='xavier_normal')
        self.init_matrix = nn.Dense(self.numOfbase, self.numOfbase, has_bias=True, weight_init='xavier_normal')
        ...
        self.merge_matrix_d = nn.Dense(self.numOfbase, self.numOfbase, has_bias=False, weight_init='xavier_normal')
        ...
        self.merge_matrix_p = nn.Dense(self.numOfbase, self.numOfbase, has_bias=False, weight_init='xavier_normal')
        ...
        self.predict_matrix = nn.Dense(self.numOfbase, self.numOfbase, has_bias=False, weight_init='xavier_normal')
        ...
        self.score_vector_1 = ms.Parameter(ms.Tensor(shape=(1, self.numOfbase), dtype=ms.float32, init=XavierNormal()),
                                           requires_grad=True)
        # nn.init.xavier_normal_(self.score_vector_1)

        self.score_vector_2 = ms.Parameter(ms.Tensor(shape=(1, self.numOfbase), dtype=ms.float32, init=XavierNormal()),
                                           requires_grad=True)
        # nn.init.xavier_normal_(self.score_vector_2)

        self.period_matrix = nn.Dense(2 * self.numOfbase, self.numOfbase, has_bias=False, weight_init='xavier_normal')
        # nn.init.xavier_normal_(self.period_matrix.weight.data)

        self.trade_vector = ms.Parameter(ms.Tensor(shape=(10, self.numOfbase), dtype=ms.float32, init=XavierNormal()),
                                         requires_grad=True)
        # nn.init.xavier_normal_(self.trade_vector)

        self.bias = ms.Parameter(ms.Tensor(shape=(1, self.numOfbase), dtype=ms.float32, init=XavierNormal()),
                                 requires_grad=True)
        # nn.init.xavier_normal_(self.bias)

        self.time_embedding = ms.Parameter(ms.Tensor(shape=(1, self.numOfbase), dtype=ms.float32, init=XavierNormal()),
                                           requires_grad=True)
        # nn.init.xavier_normal_(self.time_embedding)

        self.lamda = ms.Parameter(ms.Tensor(shape=(1, 1), dtype=ms.float32, init=XavierNormal()), requires_grad=True)
        # nn.init.xavier_normal_(self.time_embedding)
        ...
        self.entity_weights_copy = nn.Embedding(vocab_size=self.entity_weights.embedding_table.shape[0],
                                                embedding_size=self.entity_weights.embedding_table.shape[1],
                                                embedding_table=self.entity_weights.embedding_table.data)
        self.entity_weights_copy.embedding_table.requires_grad = False
        ...
        self.his_e = ms.Tensor(shape=(self.numOfEntity + 1, self.graph_sample), dtype=ms.int64, init=One())  # torch.zeros(self.numOfEntity+1, self.numOfbase, requires_grad=False).cuda()
        self.his_r = ms.Tensor(shape=(self.numOfEntity + 1, self.graph_sample), dtype=ms.int64, init=One())
        self.trend = ms.Parameter(ms.ops.zeros(self.entity_weights.embedding_table.shape, ms.float32))
        ...
        self.step_dict = ms.ops.zeros(self.numOfEntity + 1, ms.float32)
        ...
        # self.concat_a1 = ms.ops.Concat(axis=1)
        # self.concat_an1 = ms.ops.Concat(axis=-1)
        self.bmm_trans_a = ms.ops.BatchMatMul()


    # self.norm = ms.ops.LpNorm(p=2, axis=[0,1])
    ...
    def init_weights(self, weights, new_entities, related_nodes, step):
        ...
        if self.init_type == "average":
            ...
            related_node_weights = ms.ops.mean(weights[related_nodes], axis=-2)  # .unsqueeze(0).repeat(random_weights.size()[0], 1) #new_entity*graphsample*base
    ...
    def reset(self):
        ...
        self.entity_weights = nn.Embedding(self.entity_weights_copy.embedding_table.shape[0],
                                           self.entity_weights_copy.embedding_table.shape[1],
                                           embedding_table=self.entity_weights_copy.embedding_table.data)
        self.entity_weights.embedding_table.requires_grad = False

        self.his_e[:] = self.numOfEntity
        self.his_r[:] = self.numOfRelation
        # self.trend = torch.zeros(self.entity_weights.weight.data.size(), requires_grad=True).cuda()
        self.trend = ms.Parameter(ms.ops.zeros(self.entity_weights.embedding_table.data.shape, ms.float32))
        '''
        with torch.no_grad():
            self.step_dict = torch.zeros(self.numOfEntity+1, requires_grad=False).cuda()
            self.entity_weights.weight.data = self.entity_weights_copy.clone() #nn.Embedding.from_pretrained(self.entity_weights_copy.weight.data)
    ...
    def masked_maxpool(self, A):
        ...
        tmp_max, _ = ms.ops.max(A, axis=-2)[0]
        tmp_min, _ = ms.ops.min(-A, axis=-2)[0]

        # tmp_max = tmp_max-tmp_min*(tmp_max <= 0.0).float()
        tmp_max = tmp_max - tmp_min * ms.ops.cast((tmp_max <= 0.0), ms.float32)
        return tmp_max
    ...
    def bilinear_agg(self, A):
        ...
        tmp = self.bmm_trans_a(ms.ops.swapaxes(A_proj, 1,2), A)
        out = ms.ops.sum(A, 1)
    ...
    def get_time_embedding(self, time):
        ...
        in_embedding = self.time_embedding * time.unsqueeze(1) + self.bias
        cos_emb = ms.ops.cos(in_embedding).unsqueeze(1) * ((2 / self.numOfbase) ** (1 / 2))
        sin_emb = ms.ops.sin(in_embedding).unsqueeze(1) * ((2 / self.numOfbase) ** (1 / 2))

        combined = ms.ops.swapaxes(ms.ops.concat([cos_emb, sin_emb], axis=1), 2,1)
        out = ms.ops.flatten(combined)
    ...
    def construct(self, i, new_nodes, active_nodes, interact_message, \
                  prop_nodes, prop_message, \
                  pos_sample, neg_sample, steps, appeared_nodes, related_nodes):

        # tmp_weights = self.entity_weights(torch.cuda.LongTensor(self.all_ent_list))
        tmp_weights = self.entity_weights(ms.Tensor(self.all_ent_list, ms.int64))
        ...
        for step in range(len(new_nodes)):
            ...
            new_weights, norm1, norm2 = self.update_weights(tmp_weights, new_nodes[step], active_nodes[step],
                                                            interact_message[step], \
                                                            prop_nodes[step], prop_message[step], steps[step],
                                                            active_nodes[step], related_nodes[step])
            Loss += self.caculate_loss_L(new_weights, pos_sample[step][0], neg_sample[step][0], steps[step],
                                         active_nodes[step])
            if self.dataset == "YAGO":
                ...
                Loss += 0.1 * self.caculate_loss_F(new_weights, pos_sample[step][1], neg_sample[step][1],
                                                   steps[step] + 1, active_nodes[step], new_nodes[step])
            else:
                # Loss += self.caculate_loss_F(new_weights, pos_sample[step][1].cuda(), neg_sample[step][1].cuda(),
                # steps[step] + 1, active_nodes[step].cuda(), new_nodes[step].cuda())
                Loss += self.caculate_loss_F(new_weights, pos_sample[step][1], neg_sample[step][1], steps[step] + 1,
                                             active_nodes[step], new_nodes[step])
        ...
        # 	self.entity_weights.weight.requires_grad = False
        # self.entity_weights.embedding_table = ms.ops.stop_gradient(self.entity_weights.embedding_table)
        # tmp_weights_detach = ms.Parameter(tmp_weights, requires_grad=False)
        self.entity_weights.embedding_table[0:self.numOfEntity] = tmp_weights[0:self.numOfEntity].copy() # .clone() #.detach()
        self.entity_weights.embedding_table.requires_grad = False
        ...
        # 	norm_loss += W.norm(2)
        for W in self.gate_matrix.get_parameters():
            norm_loss += W.norm(2)
    ...
    def update_weights(self, weights, new_entities, active_nodes, interact_message, prop_nodes, prop_message, step,
                       appeared_nodes, related_nodes):
        ...
        inter_e_weights = self.read_weights(weights[interact_message[:, 1].flatten()]).view(-1, self.graph_sample,
                                                                                            self.numOfbase)  # self.get_weights(weights, interact_message_b[:,1].flatten(), step, appeared_nodes).view(-1, self.graph_sample, self.numOfbase)
        ...
        interact_information = self.interact(ms.ops.tile(active_nodes_weights.unsqueeze(1), (1, self.graph_sample, 1)),
                                             inter_e_weights,
                                             rel_embedding)  # self.ccorr(inter_e_weights_build.view(-1,self.numOfbase), rel_1_embedding.view(-1,self.numOfbase)).view(rel_2_embedding.size()) #inter_e_weights_build - rel_b_embedding #rel_b_embedding*inter_e_weights_build #rel_b_embedding*read_gate*inter_e_weights_build #torch.tanh(self.build_matrix(torch.cat([rel_b_embedding, inter_e_weights_build], -1))) #self.combiner_b(inter_e_weights_build, rel_b_embedding, active_nodes_weights.unsqueeze(1).repeat(1,self.graph_sample,1), step-self.step_dict[active_nodes])*(inter_e_weights_build.sum(2)!=0).unsqueeze(2).float() #torch.tanh(self.dir_influence_matrix(torch.cat([rel_b_embedding,inter_e_weights_build], -1)))*(inter_e_weights_build.sum(2)!=0).unsqueeze(2).float()

        interact_e = interact_message[:, 1].view(len(active_nodes), -1)
        # interact_r = torch.abs(interact_message[:,0]).view(len(active_nodes), -1)
        interact_r = ms.ops.abs(interact_message[:, 0]).view(len(active_nodes), -1)
        ...
        direct_influence = ms.ops.sum(interact_information,
        ...
        ent_weights = self.read_weights(weights[prop_message[:, :, 2].flatten()]).view(-1, self.graph_sample,
                                                                                       self.numOfbase)  # self.get_weights(weights, prop_message_b[:,:,0].flatten(), step, appeared_nodes).view(-1, self.graph_sample, self.numOfbase)
        ...
        rel_1_embedding = self.get_rel_weights(prop_message[:, :, 0],
                                               step)  # .detach() # * (torch.sign(prop_message_b[:,:,0]).float()+(prop_message_b[:,:,0]==0).float()).unsqueeze(-1)
        rel_2_embedding = self.get_rel_weights(prop_message[:, :, 1],
                                               step)  # .detach() # * (torch.sign(prop_message_b[:,:,1]).float()+(prop_message_b[:,:,1]==0).float()).unsqueeze(-1)
        ...
        prop_information = self.tanh(self.prop_influence_matrix(ms.ops.concat([rel_1_embedding, rel_2_embedding,
                                                                                 ent_weights], axis=-1)))  # *(related_node_weights.sum(2)!=0).unsqueeze(2).float() #torch.tanh(self.prop_influence_matrix(interact_information)*((prop_nodes_weights_trade.unsqueeze(1)+related_node_weights)*(related_node_weights.sum(2)!=0).unsqueeze(2).float()))
        ...
        prop_influence = ms.ops.sum(prop_information, 1)  # /((ent_weights.sum(2)!=0).sum(1).float() + ((ent_weights.sum(2)!=0).sum(1)==0).float()).unsqueeze(1)
        ...
        active_nodes = active_nodes.asnumpy().tolist()
        prop_nodes = prop_nodes.asnumpy().tolist()
        ...
        if len(corres_nodes) != 0:
            ...
            cur_weights_p = ms.ops.zeros(cur_weights_d.shape, ms.float32)  # prop_influence[corres_nodes_index_p]
            # print(cur_weights_p[0][0:10])
            # cur_weights = torch.tanh(cur_weights_d)
            new_corr = self.update(cur_weights_d, cur_weights_p,
                                   corr_weights)  # (1-gate)*corr_weights + gate*cur_weights
            ...
            # norm1 += self.norm(pre_weights_trade - new_corr.detach()).mean()
            # new_corr_detach = ms.Parameter(new_corr, requires_grad=False)
            norm1 += ms.ops.norm(pre_weights_trade - new_corr.copy()).mean()    #, ord='fro'
            norm2 += ms.ops.norm(corr_weights - new_corr).mean()
        ...
        if len(only_interact_nodes) != 0:
            ...
            pre_weights_trade = self.get_weights(weights, only_interact_nodes, step,
                                                 appeared_nodes)  # self.entity_weights_copy(torch.cuda.LongTensor(only_interact_nodes))
            cur_weights_d = direct_influence[only_interact_index]
            # cur_weights_p = torch.zeros(cur_weights_d.size(), requires_grad=False).cuda()
            cur_weights_p = ms.ops.zeros(cur_weights_d.shape, ms.float32)
            # gate = torch.sigmoid(self.gate_matrix(torch.cat([inter_weights, cur_weights], 1)))
            # cur_weights = torch.tanh(cur_weights_d)
            new_inter = self.update(cur_weights_d, cur_weights_p,
                                    inter_weights)  # cur_weights #(1-gate)*inter_weights + gate*cur_weights
            ...
            # new_inter_detach = ms.Parameter(new_inter, requires_grad=False)
            norm1 += ms.ops.norm(pre_weights_trade - new_inter.copy()).mean()  # .sum()
            norm2 += ms.ops.norm(inter_weights - new_inter).mean()
        ...
        if len(only_prop_nodes) != 0:
            ...
            cur_weights_d = ms.ops.zeros(cur_weights_p.shape, ms.float32)
            # cur_weights = torch.tanh(cur_weights_p)
            new_prop = self.update(cur_weights_p, cur_weights_p,
                                   prop_weights)  # (1-gate)*prop_weights + gate*cur_weights
            ...
            norm1 += ms.ops.norm(pre_weights_trade - new_prop).mean()
            norm2 += ms.ops.norm(prop_weights - new_prop).mean()
    ...
    def get_weights(self, weights, index, step, all_ent):
        ...
        related_e_weights = weights[related_e.flatten()].view(related_e.shape[0], related_e.shape[1], -1)
        ...
        time_span = ms.ops.tile((step - self.step_dict[related_e.flatten()]).view(related_e.shape[0], related_e.shape[1]).unsqueeze(-1), (1,1,self.numOfbase))
        ...
        related_e_weights = ms.ops.mean(related_e_weights * ms.ops.sigmoid(time_span * self.lamda), 1)
        ...
        periods = ms.Tensor([(i + 1) ** 2 for i in range(self.trade_vector.shape[0])], ms.float32)
        t_tmp = (self.trade_vector * step) // periods.unsqueeze(-1)
        ...
        t = ms.ops.mean(ms.ops.sin(self.period_matrix(ms.ops.concat(
            [ms.ops.tile(raw_weights.unsqueeze(1), (1,self.trade_vector.shape[0],1)),
             ms.ops.tile(t_tmp.unsqueeze(0), (raw_weights.shape[0], 1, 1))], axis=-1))), 1)
    ...
    def caculate_loss_L(self, weights, triples, neg_sample, step, appeared_nodes):
        ...
        neg_score_h = self.score_func(neg_h_embedding,
                                      ms.ops.tile(pos_r_embedding.unsqueeze(1),(1,self.ns,1)).view(-1, self.numOfbase),
                                      ms.ops.tile(pos_t_embedding.unsqueeze(1),(1,self.ns,1)).view(-1,self.numOfbase)).view(-1, self.ns)  # .detach()
        ...
        neg_score_t = self.score_func(ms.ops.tile(pos_h_embedding.unsqueeze(1),(1,self.ns,1)).view(-1, self.numOfbase),
                                      ms.ops.tile(pos_r_embedding.unsqueeze(1),(1,self.ns,1)).view(-1, self.numOfbase),
                                      neg_t_embedding).view(-1, self.ns)  # .detach()
        ...
        norm_loss += ms.ops.norm(pos_r_embedding)  # + torch.norm(pos_h_embedding) + Loss.normLoss(pos_t_embedding)# + Loss.normLoss(neg_t_embedding,1) #torch.mean(pos_h_embedding ** 2) + torch.mean(pos_r_embedding ** 2) + torch.mean(pos_t_embedding ** 2) + torch.mean(neg_h_embedding ** 2) + torch.mean(neg_t_embedding ** 2)
    ...
    def caculate_loss_F(self, weights, triples, neg_sample, step, appeared_nodes, new_entities):
        ...
        # print(weights_clone[0][0:10])
        # weights[ents] = weights[ents] + torch.tanh(self.project_matrix(weights[ents]))*0.01*(step-self.step_dict[ents]).unsqueeze(1)#.clone()#.requires_grad_()
        all_embeddings = self.get_weights(weights_clone, self.all_ent_list, step,
                                          appeared_nodes)  # self.act(self.base_matrix(self.get_weights(weights_clone,self.all_ent_list,step,appeared_nodes)))
        ...
        # print(neg_sample.size())
        neg_h_embedding = all_embeddings[neg_sample[
            0]]  # .view(pos_h_embedding.size()[0],-1,self.numOfhidden)#.detach() #self.base_matrix(weights[neg_sample[0]]).view(pos_h_embedding.size()[0],-1,self.numOfhidden)
        # neg_r_embedding = self.relation_embeddings(neg_sample[1]).view(-1,self.numOfhidden)
        neg_t_embedding = all_embeddings[neg_sample[
            2]]  # .view(pos_h_embedding.size()[0],-1,self.numOfhidden)#.detach() #self.base_matrix(weights[neg_sample[2]]).view(pos_h_embedding.size()[0],-1,self.numOfhidden)
        ...
        neg_score_h = self.score_func(neg_h_embedding,
                                      ms.ops.tile(pos_r_embedding.unsqueeze(1),(1,self.ns,1)).view(-1, self.numOfbase),
                                      ms.ops.tile(pos_t_embedding.unsqueeze(1),(1,self.ns,1)).view(-1,
                                                                                                  self.numOfbase)).view(
            -1, self.ns)  # .detach()
        # neg_score_r = self.score_func(pos_h_embedding.unsqueeze(1).repeat(1,10,1).view(-1,self.numOfbase),neg_r_embedding,pos_t_embedding.unsqueeze(1).repeat(1,10,1).view(-1,self.numOfbase)).view(-1,10) #.sigmoid()
        # neg_score_t = self.score_func(pos_h_embedding.unsqueeze(1).repeat(1,self.ns,1).view(-1,self.numOfbase),pos_r_embedding.unsqueeze(1).repeat(1,self.ns,1).view(-1,self.numOfbase),neg_t_embedding).view(-1,self.ns) #.detach()
        ...
        norm_loss += ms.ops.norm(pos_r_embedding)  # + torch.norm(pos_t_embedding) + torch.norm(pos_h_embedding)# + Loss.normLoss(pos_h_embedding,1) + Loss.normLoss(pos_t_embedding,1) + Loss.normLoss(neg_h_embedding,1) + Loss.normLoss(neg_t_embedding,1) #torch.mean(pos_h_embedding ** 2) + torch.mean(pos_r_embedding ** 2) + torch.mean(pos_t_embedding ** 2) + torch.mean(neg_h_embedding ** 2) + torch.mean(neg_t_embedding ** 2)
        # norm_loss += self.update_matrix.weight.norm(2) + self.build_matrix.weight.norm(2) + self.prop_matrix.weight.norm(2)
    ...
    def forecast(self, weights, all_ent, triples, step, appeared_nodes, new_entities,
                 batch_data):  # low efficency because of the gpu memory
        ...
        all_embeddings = self.get_weights(weights_clone, self.all_ent_list, step,
                                          appeared_nodes)  # self.act(self.base_matrix(self.get_weights(weights_clone,self.all_ent_list,step,appeared_nodes))) #self.base_matrix(weights[0:self.numOfEntity])
        ...
        for triple in triples:
            ...
            tmp_h_embedding = ms.ops.tile(h_embedding, (len(all_embeddings), 1))
            tmp_r_embedding = ms.ops.tile(r_embedding, (len(all_embeddings), 1))
            tmp_t_embedding = ms.ops.tile(t_embedding, (len(all_embeddings), 1))

            pos_score = self.score_func(tmp_h_embedding, tmp_r_embedding,
                                        tmp_t_embedding)  # torch.sum(tmp_h_embedding*tmp_t_embedding*tmp_r_embedding, 1)#.sigmoid()
            neg_h_score = self.score_func(all_embeddings, tmp_r_embedding,
                                          tmp_t_embedding)  # torch.sum(appeared_embeddings*tmp_t_embedding*tmp_r_embedding, 1)#.sigmoid()
            neg_t_score = self.score_func(tmp_h_embedding, tmp_r_embedding,
                                          all_embeddings)  # torch.sum(tmp_h_embedding*appeared_embeddings*tmp_r_embedding, 1)#.sigmoid()
            ...
            label_h = ms.ops.zeros(len(all_embeddings), ms.float32)
            label_h[all_ent] = 1
            label_h[filter_h] = 0

            # label_t = torch.zeros(len(all_embeddings), requires_grad=False).cuda()
            label_t = ms.ops.zeros(len(all_embeddings), ms.float32)
            label_t[all_ent] = 1
            label_t[filter_t] = 0

            # wrongHead = torch.nonzero(self.relu(tmp_head*label_h))
            # wrongTail = torch.nonzero(self.relu(tmp_tail*label_t))
            wrongHead = ms.ops.nonzero(self.relu(tmp_head * label_h))
            wrongTail = ms.ops.nonzero(self.relu(tmp_tail * label_t))

            # Rank_H = wrongHead.size()[0] + 1
            # Rank_T = wrongTail.size()[0] + 1
            Rank_H = wrongHead.shape[0] + 1
            Rank_T = wrongTail.shape[0] + 1
    ...
    def reconstruct(self, weights, all_ent, triples, step, appeared_nodes):
        ...
        # print(all_embeddings[0][0:10])
        # all_embeddings = F.normalize(all_embeddings)
        appeared_embeddings = weights[
            all_ent]  # self.act(self.base_matrix(self.get_weights(weights_clone,all_ent,step,appeared_nodes)))
        # appeared_embeddings = F.normalize(appeared_embeddings)
        # print(len(all_ent))
        ...
        for triple in triples:
            ...
            tmp_h_embedding = ms.ops.tile(h_embedding, (len(all_ent), 1))
            tmp_r_embedding = ms.ops.tile(r_embedding, (len(all_ent), 1))
            tmp_t_embedding = ms.ops.tile(t_embedding, (len(all_ent), 1))

            pos_score = self.score_func(tmp_h_embedding, tmp_r_embedding,
                                        tmp_t_embedding)  # torch.sum(tmp_h_embedding*tmp_t_embedding*tmp_r_embedding, 1)#.sigmoid()
            neg_h_score = self.score_func(appeared_embeddings, tmp_r_embedding,
                                          tmp_t_embedding)  # torch.sum(appeared_embeddings*tmp_t_embedding*tmp_r_embedding, 1)#.sigmoid()
            neg_t_score = self.score_func(tmp_h_embedding, tmp_r_embedding,
                                          appeared_embeddings)  # torch.sum(tmp_h_embedding*appeared_embeddings*tmp_r_embedding, 1)#.sigmoid()
            # pos_score = torch.norm((tmp_h_embedding - tmp_t_embedding)+tmp_r_embedding, 2, 1)#.sigmoid()
            # neg_h_score = torch.norm((appeared_embeddings - tmp_t_embedding)+tmp_r_embedding, 2, 1)#.sigmoid()
            # neg_t_score = torch.norm((tmp_h_embedding - appeared_embeddings)+tmp_r_embedding, 2, 1)#.sigmoid()
            # pos_score = trch.sum(self.predict_vector.unsqueeze(1)*torch.tanh(self.predict_matrix(torch.cat([tmp_h_embedding, tmp_r_embedding, tmp_t_embedding], -1))), 1)
            ...
            wrongHead = ms.ops.nonzero(self.relu(tmp_head))
            wrongTail = ms.ops.nonzero(self.relu(tmp_tail))
    ...
    def masked_softmax(self, A, dim):
        ...
        A_max, _ = ms.ops.max(A, axis=dim, keepdims=True)[0]
        A_exp = ms.ops.exp(A - A_max)
        A_exp = A_exp * ms.ops.cast((A != 0), ms.float32)
        Sum = ms.ops.sum(A_exp, dim=dim, keepdim=True)
    ...
    def com_mult(self, a, b):
        ...
        out = ms.ops.stack([r1 * r2 - i1 * i2, r1 * i2 + i1 * r2], -1)
    ...
    def ccorr(self, a, b):
        rfft_ms = ms.ops.FFTWithSize(signal_ndim=1, inverse=False, real=True)
        irfft_ms = ms.ops.FFTWithSize(signal_ndim=1, inverse=True, real=True, signal_sizes=(a.shape[-1],))
        return irfft_ms(self.com_mult(self.conj(rfft_ms(a)), rfft_ms(b)))
    ...
    def get_rel_weights(self, index, step):
        rel_emb_1 = self.relation_embeddings_1(ms.ops.abs(index))
...
class GRUCell(nn.Cell):
    ...
    def __init__(self, input_size, hidden_size, his_length, bias):
        ...
        self.update_gate = nn.Dense(2 * input_size, hidden_size, has_bias=bias, weight_init='xavier_normal')
        self.reset_gate = nn.Dense(2 * input_size, hidden_size, has_bias=bias, weight_init='xavier_normal')
        self.new_gate = nn.Dense(2 * input_size, hidden_size, has_bias=bias, weight_init='xavier_normal')
        self.merge_matrix = nn.Dense(2 * input_size, hidden_size, has_bias=bias, weight_init='xavier_normal')
        # nn.init.xavier_normal_(self.merge_matrix.weight.data)
        self.his_length = his_length
    ...
    def construct(self, x1, x2, hidden):
        ...
        z = ms.ops.sigmoid(self.update_gate(ms.ops.concat([x, hidden], axis=-1)))
        r = ms.ops.sigmoid(self.reset_gate(ms.ops.concat([x, hidden], axis=-1)))
        h = ms.ops.sigmoid(self.new_gate(ms.ops.concat([x, r * hidden], axis=-1)))
...
class updater(nn.Cell):
    ...
    def __init__(self, input_size, hidden_size, his_length, bias):
        ...
        self.new_gate = nn.Dense(2 * input_size, hidden_size, has_bias=bias, weight_init='xavier_normal')
        # nn.init.xavier_normal_(self.new_gate.weight.data)
        self.inter_gate = nn.Dense(input_size, hidden_size, has_bias=bias, weight_init='xavier_normal')
        # nn.init.xavier_normal_(self.inter_gate.weight.data)
        self.his_length = his_length

        self.lamda = self.time_embedding = ms.Parameter(ms.Tensor(shape=(1, 1), dtype=ms.float32, init=XavierNormal()))
        # nn.init.xavier_normal_(self.time_embedding)
    ...
    def construct(self, x1, x2, hidden):
        ...
        x = x1  # + x2
        z = ms.ops.sigmoid(self.update_gate(ms.ops.concat([x, hidden], -1)))  # *torch.exp(-span).unsqueeze(1)#.unsqueeze(1)
        # z = F.normalize(z,2,1)
        # r = self.sigmoid(self.reset_gate(torch.cat([x,hidden],-1)))#*torch.exp(-span).unsqueeze(1)#.unsqueeze(1)
        h = self.tanh(self.new_gate(ms.ops.concat([x, hidden], -1)))
...
class ConvTransE(nn.Cell):
    def __init__(self):
        super(ConvTransE, self).__init__()
        self.inp_drop = nn.Dropout(0)
        self.hidden_drop = nn.Dropout(0.25)
        self.feature_map_drop = nn.Dropout(0.25)

        self.conv1 = nn.Conv1d(2, 3, 5, stride=1, has_bias=True, padding=int(math.floor(5 / 2)),
                               pad_mode='pad')  # kernel size is odd, then padding = math.floor(kernel_size/2)
        self.bn0 = nn.BatchNorm1d(200, momentum=0.9)
        self.bn1 = nn.BatchNorm1d(300, momentum=0.9)
        self.bn2 = nn.BatchNorm1d(100, momentum=0.9)
        self.fc = nn.Dense(600, 200)
        self.bn_init = nn.BatchNorm1d(100, momentum=0.9)

        self.tanh = nn.Tanh()

    def construct(self, e1, rel, X):
        ...
        e1_embedded = e1.unsqueeze(1)  # self.bn_init(e1)
        rel_embedded = rel.unsqueeze(1)
        stacked_inputs = ms.ops.concat([e1_embedded, rel_embedded], axis=1)
        ...
        # x = self.bn2(x)
        x = self.tanh(x)
        x = ms.ops.sum(x * e1_embedded_all, dim=1)  # .view(-1)
        # print(x.size())
...
#         #x = self.bn2(x)
#         x = self.tanh(x)
#         x = self.sum(x*e1_embedded_all,axis=1)#.view(-1)
#         #print(x.size())
#
#         return x
...
class DistMult(nn.Cell):
    def __init__(self):
        ...
        # self.trans = nn.Dense(200, 100)
        self.inp_drop = nn.Dropout(0.4)
    ...
    def construct(self, s, r, o):
        ...
        score = ms.ops.sum(self.inp_drop(s * r * o), dim=1)
...
class ConvE(nn.Cell):
    def __init__(self, embedding_dim):
        ...
        self.conv1 = nn.Conv2d(1, 8, (3, 3), 1, 0, has_bias=True, pad_mode='pad')
        self.bn0 = nn.BatchNorm2d(1, momentum=0.9)
        self.bn1 = nn.BatchNorm2d(8, momentum=0.9)
        self.bn2 = nn.BatchNorm1d(embedding_dim, momentum=0.9)
        self.fc = nn.Dense(1152, embedding_dim)
        # self.register_parameter('b', Parameter(torch.zeros(num_entities)))

        self.inp_drop = nn.Dropout(0.3)
        self.hidden_drop = nn.Dropout(0.2)
        self.feature_map_drop = nn.Dropout2d(0.2)

        self.relu = nn.ReLU()

    def construct(self, e1, rel, e2):
        e1_embedded = e1.view(-1, 1, 10, 10)
        rel_embedded = rel.view(-1, 1, 10, 10)

        stacked_inputs = ms.ops.concat([e1_embedded, rel_embedded], axis=2)
...
# 		# self.register_parameter('b', Parameter(torch.zeros(num_entities)))
#
# 		self.inp_drop = nn.Dropout(0.7)
# 		self.hidden_drop = nn.Dropout(0.8)
# 		self.feature_map_drop = nn.Dropout2d(0.8)
#
# 		self.concat = ms.ops.Concat(axis=2)
# 		self.relu = nn.ReLU()
...
#         rel_embedded = rel.view(-1, 1, 10, 10)
#
#         stacked_inputs = self.concat([e1_embedded, rel_embedded])
#
#         x = self.bn0(stacked_inputs)
#         x= self.inp_drop(x)
#         x= self.conv1(x)
