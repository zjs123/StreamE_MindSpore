...
import numpy as np
import random as rd
from Model import Model, MyTrainOneStepCell
...
class Main:
    ...
    def __init__(self, args):
        ...
        raw_ent = []
        raw_rel = []
        self.model = Model(self.data_dict["entity_num"], self.data_dict["relation_num"], self.hidden, self.dataset,
                           raw_ent, raw_rel, args.init_type)
        # self.model = auto_mixed_precision(self.model, 'O2')
    ...
    def Train(self):
        ...
        optimizer = nn.Adam(self.model.trainable_params(), lr=self.lr, weight_decay=0, eps=1e-2)
        ms_model = MyTrainOneStepCell(self.model, optimizer)
        ...
        for epoch in range(self.numOfEpoch):
            ...
            p = progressbar.ProgressBar(
                widgets=["Epoch", str(epoch), ":[", progressbar.Percentage(), "]", progressbar.Timer()],
                maxval=self.train_time - 1)
            p.start()
            self.DataLoader.reset()
            # self.model.reset()
            # self.model.train()
            ms_model.network.reset()
            ms_model.network.set_train()
            ...
            while t < self.train_time - 1:
                ...
                for i in range(self.minibatch):
                    if t + i < self.train_time - 1:
                        ...
                        neg_sample.append(self.DataLoader.negativeSampling(triples, batch_data["active_nodes"],
                                                                           batch_data["prop_nodes"], t))
                        steps.append(t + i)
                    else:
                        break
                ...
                batchLoss = ms_model(t, new_nodes, active_nodes, interact_message, \
                                               prop_nodes, prop_message, \
                                               pos_sample, neg_sample, steps, appeared_nodes, related_nodes)
            ...
            if epoch % 1 == 0:
                # self.model.eval()
                ms_model.network.set_train(False)
                # with torch.no_grad():
                    # print("evaluating...")
                self.eval(self.model.entity_weights.embedding_table.data)
                print(self.best)
    ...
    def save_embedding(self):
        ...
        ent_path = base_path + "ent_raw_B_50.ckpt"
        rel_path = base_path + "rel_raw_B_50.ckpt"
        ms.save_checkpoint(self.model.entity_weights.embedding_table.data, ent_path)
        ms.save_checkpoint(self.model.relation_embeddings.embedding_table.data, rel_path)
        print("saved")
    ...
    def get_embdding(self):
        ...
        ent_path = base_path + "ent_raw_B_50.ckpt"
        rel_path = base_path + "rel_raw_B_50.ckpt"
        ent_emb = ms.load_checkpoint(ent_path)
        rel_emb = ms.load_checkpoint(rel_path)
    ...
    def eval(self, pre_weights):
        ...
        p = progressbar.ProgressBar(
            widgets=["Forecast:", progressbar.Bar('*'), progressbar.Percentage(), "|", progressbar.Timer()],
            maxval=self.all_time - start_time - 1)
        p.start()
        numOftriples = 0
        # tmp_weights = pre_weights.clone().detach()
        tmp_weights = pre_weights.clone()
        for batch in range(self.all_time - start_time - 1):
            ...
            new_nodes = batch_data["new_nodes"]
            appeared_nodes = batch_data["appeared_nodes"]
            active_nodes = batch_data["active_nodes"]
            interact_message = batch_data["interact_message"]
            prop_nodes = batch_data["prop_nodes"]
            prop_message = batch_data["prop_message"]
            related_nodes = batch_data["related_nodes"]


            new_weights, norm1, norm2 = self.model.update_weights(tmp_weights, new_nodes, active_nodes,
                                                                  interact_message, \
                                                                  prop_nodes, prop_message, batch_time, active_nodes,
                                                                  related_nodes)

            # print(new_weights[0][0:10])
            if batch_time in self.data_dict["test_triples"].keys():
                numOftriples += len(triples[1])
                # result = self.model.reconstruct(new_weights, appeared_nodes.cuda(), triples[0].cuda(), batch_time,
                #                                 appeared_nodes)
                result = self.model.reconstruct(new_weights, appeared_nodes, triples[0], batch_time,
                                                appeared_nodes)
                MRR[0] += result[0]
                H1[0] += result[1]
                H3[0] += result[2]
                ...
                # result = self.model.forecast(new_weights, appeared_nodes.cuda(), triples[1].cuda(), batch_time + 1,
                #                              active_nodes, new_nodes, self.data_dict["T-e-r-e"][batch_time + 1])
                result = self.model.forecast(new_weights, appeared_nodes, triples[1], batch_time + 1,
                                             active_nodes, new_nodes, self.data_dict["T-e-r-e"][batch_time + 1])
                MRR[1] += result[0]
                H1[1] += result[1]
                H3[1] += result[2]
...
if __name__ == '__main__':
    ...
    ms.set_context(device_target='GPU', device_id=1, mode=ms.PYNATIVE_MODE)
