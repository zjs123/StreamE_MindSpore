...
class DataLoader:
    ...
    def generateBatches(self, batch, Type):
        ...
        batch_data["related_nodes"] = ms.Tensor(list(related_nodes), ms.int64)

        # batch_data["new_nodes"] = torch.LongTensor(list(new_nodes))
        batch_data["new_nodes"] = ms.Tensor(list(new_nodes), ms.int64)

        # find active nodes
        # batch_data["active_nodes"] = torch.LongTensor(list(self.data_dict["T-e-r-e"][batch].keys()))
        batch_data["active_nodes"] = ms.Tensor(list(self.data_dict["T-e-r-e"][batch].keys()), ms.int64)
        ...
        batch_data["interact_message"] = ms.Tensor(batch_data["interact_message"], ms.int64)

        # find prop nodes
        # batch_data["prop_nodes"] = torch.LongTensor(list(prop_nodes_2_message.keys()))
        batch_data["prop_nodes"] = ms.Tensor(list(prop_nodes_2_message.keys()), ms.int64)

        # generate prop message
        batch_data["prop_message"] = []
        if len(prop_nodes_2_message.keys()) == 0:
            batch_data["prop_message"].append(
                [[self.numOfRelation, self.numOfRelation, self.numOfEntity]] * self.graph_sample)
        else:
            for node in list(prop_nodes_2_message.keys()):
                if len(prop_nodes_2_message[node]) < self.graph_sample:
                    ...
                    message_b.extend([[self.numOfRelation, self.numOfRelation, self.numOfEntity]] * (
                        self.graph_sample - len(prop_nodes_2_message[node])))
        ...
        batch_data["prop_message"] = ms.Tensor(batch_data["prop_message"], ms.int64)  # prop*sample*3

        # batch_data["appeared_nodes"] = torch.LongTensor(list(self.appeared_nodes))
        batch_data["appeared_nodes"] = ms.Tensor(list(self.appeared_nodes), ms.int64)
        ...
        if Type == "Train":
            ...
            triples.append(ms.Tensor(self.data_dict["all_triples"][batch], ms.int64))
            triples.append(ms.Tensor(self.filter(self.data_dict["all_triples"][batch + 1]), ms.int64))
            ...
            triples.append(ms.Tensor(self.data_dict["all_triples"][batch], ms.int64))
            triples.append(ms.Tensor(self.filter_unseen(self.data_dict["all_triples"][batch + 1]), ms.int64))
    ...
    def negativeSampling(self, triples, active_nodes, prop_nodes, batch):
        ...
        neg_r_2 = []
        neg_samples = []
        active_nodes = active_nodes.asnumpy()
        prop_nodes = prop_nodes.asnumpy()
        ...
        for triple in triples[0]:
            ...
            candidate_nodes_h = now_nodes - set([head]) - set(
                self.data_dict["e-e-[r]"][tail].keys())  # list(torch.cat([active_nodes, prop_nodes]))
            candidate_nodes_t = now_nodes - set([tail]) - set(
                self.data_dict["e-e-[r]"][head].keys())  # - set(sum(self.data_dict["e-r-[e]"][tail][-rel], []))
        ...
        neg_samples.append(ms.Tensor([neg_h_1, neg_r_1, neg_t_1], ms.int64))
        ...
        for triple in triples[1]:
            ...
            candidate_nodes_h = next_nodes - set([head]) - set(
                self.data_dict["e-e-[r]"][tail].keys())  # list(torch.cat([active_nodes, prop_nodes]))
        ...
        neg_samples.append(ms.Tensor([neg_h_2, neg_r_2, neg_t_2], ms.int64))
